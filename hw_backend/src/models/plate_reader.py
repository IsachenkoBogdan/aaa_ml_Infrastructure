...
class PlateReader(nn.Module):
    def __init__(
        ...
        self.cnn = nn.Conv1d(
            in_channels=512, kernel_size=3, padding=1, out_channels=N_LETTERS
        )
        self.relu = nn.ReLU()
    ...
    def read_text(self, image: BytesIO) -> str:
        ...
        image = norm(image.float() / 255.0)
        image = image.to(DEVICE)
        with torch.no_grad():
            val_preds = self.forward(image.unsqueeze(0))
            y_pred = torch.argmax(val_preds, dim=1)
            res = "".join([idx_letter_map[j] for j in y_pred.cpu()[0].numpy()])
